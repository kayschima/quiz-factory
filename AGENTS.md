...
- `wayfinder-development` — Activates whenever referencing backend routes in frontend components. Use when importing
  from @/actions or @/routes, calling Laravel routes from TypeScript, or working with Wayfinder route functions.
...
This application uses a Service Layer pattern to separate business logic from controllers. Services are located in
`app/Services/`:

- **QuestionService** - Handles all question-related business logic (CRUD, filtering, pagination)
- **QuizService** - Manages quiz logic (fetching random questions, shuffling answers)
- **ProfileService** - User profile operations (update, delete, password changes)
...
Business logic should always be placed in the appropriate service class.
...
Wayfinder generates TypeScript functions for Laravel routes. Import from `@/actions/` (controllers) or `@/routes/` (
named routes).
