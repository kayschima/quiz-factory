...
<template>
    ...
    <div class="min-h-screen flex items-center justify-center p-6 bg-white dark:bg-[#0a0a0a]">
        <div class="w-full max-w-2xl rounded-lg border p-6 space-y-6 dark:border-[#2a2a29]">
            ...
            <form @submit.prevent="submit" class="space-y-4">
                <div>
                    ...
                    />
                    <div v-if="form.errors.text" class="text-red-500 text-xs mt-1">{{ form.errors.text }}</div>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            required
                            class="w-full border rounded px-3 py-2 dark:bg-[#161615] dark:border-[#2a2a29]"
                            :class="{ 'border-red-500': form.errors.difficulty_id }"
                        >
                            <option value="">Bitte wählen</option>
                            <option v-for="diff in difficulties" :key="diff.id" :value="diff.id">{{ diff.name }}</option>
                        </select>
                        <div v-if="form.errors.difficulty_id" class="text-red-500 text-xs mt-1">{{ form.errors.difficulty_id }}</div>
                    </div>
                </div>

                <fieldset class="space-y-3">
                    <legend class="text-sm font-medium">Antworten (mind. 1, max. 3 richtig)</legend>
                    <div v-if="form.errors.answers" class="text-red-500 text-xs">{{ form.errors.answers }}</div>
                    <div v-for="(answer, index) in form.answers" :key="index" class="flex flex-col gap-1">
                        <div class="flex items-center gap-3">
                            <input
                                ...
                                type="text"
                                required
                                class="flex-1 border rounded px-3 py-2 dark:bg-[#161615] dark:border-[#2a2a29]"
                                :placeholder="`Antwort ${index + 1}`"
                                :class="{ 'border-red-500': form.errors[`answers.${index}.text`] }"
                            />
                        ...
                        <div v-if="form.errors[`answers.${index}.text`]" class="text-red-500 text-xs">{{ form.errors[`answers.${index}.text`] }}</div>
                    </div>
                </fieldset>
                ...
                <div class="flex gap-3 pt-2">
                    <button
                        ...
                        class="rounded bg-black px-4 py-2 text-white dark:bg-white dark:text-black disabled:opacity-50"
                    >
                        Speichern
                    </button>
